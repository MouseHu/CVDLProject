...
train_data = SynthiaDataset(phase='train')
train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=8)

val_data = SynthiaDataset(phase='val', flip_rate=0)
...
def train():
    for epoch in range(epochs):
	    ...
	    inputs2=torch.zeros(inputs.shape[0],inputs.shape[1],768,960)
	    inputs2[:,:,24:-24,:]=inputs
            outputs = fcn_model(inputs2)
	    outputs=outputs[:,:,24:-24,:]
