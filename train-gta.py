...
from GTA_Loader import GTAVDataset
...
vgg_model = VGGNet( model='vgg19',requires_grad=True,remove_fc=True,dataset="gtav")#####change to 19
...
def train():
    for epoch in range(epochs):
	    ...
	    inputs2=torch.zeros(inputs.shape[0],inputs.shape[1],544,960)
	    
	    inputs2[:,:,9:-9,2:-2]=inputs
            outputs = fcn_model(inputs2)
	    outputs=outputs[:,:,9:-9,2:-2]
...
def val(epoch):
	...
	inputs2=torch.zeros(inputs.shape[0],inputs.shape[1],1088,1920)
	pad_h=(1088-inputs.shape[2])/2
	pad_w=(1920-inputs.shape[3])/2
	inputs2[:,:,pad_h:-pad_h,pad_w:-pad_w]=inputs
        output = fcn_model(inputs2)
	output=output[:,:,pad_h:-(pad_h),pad_w:-(pad_w)]
